...
from rdbdiff.third_party.clava_ddpm.pipeline_utils import load_multi_table
from rdbdiff.third_party.clava_ddpm.report_utils import (
...
def recursive_merge(dataframes, keys):
    ...
    for i in range(
        ...
        # resolving a conflict happenning in rel-trial
        # TODO: this is a general issue, fixable by avoiding same column names in different tables
        if "nct_id_x" in result_df and "nct_id_y" in result_df:
            assert (result_df["nct_id_x"] == result_df["nct_id_y"]).all()
            result_df["nct_id"] = result_df["nct_id_x"]
            result_df.drop(columns=["nct_id_x", "nct_id_y"], inplace=True)
        result_df = pd.merge(
...
def gen_multi_report(real_data_path, syn_data_path, metadata_dir, eval_save_dir):
    ...
    multi_report = multi_eval_quality(real_tables, syn_tables, multi_metadata)
    ...
    pickle.dump(result, open(os.path.join(eval_save_dir, "eval_result.pkl"), "wb"))
    return result
